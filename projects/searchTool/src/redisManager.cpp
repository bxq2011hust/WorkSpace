...
bool RedisManager::selectDb(int index)
{
    ...
    mtx.unlock();
    bool success = (replay && (string("OK") == replay->str));
...
int RedisManager::set_adds(const char *keyOfset, const char *items)
{
    ...
    mtx.unlock();
    unsigned set_size = replay ? replay->integer : 0;
    freeReplyObject(replay);
    return set_size;
}
...
bool RedisManager::set_IsMember(const char *keyOfset, const char *item)
{
    ...
    mtx.unlock();
    bool success = (replay && (replay->integer == 1));
    freeReplyObject(replay);
    return success;
}
...
vector<string> RedisManager::set_getAll(const char *keyOfset)
{
    ...
    mtx.unlock();
    vector<string> res;
    if (replay && (replay->type == REDIS_REPLY_ARRAY))
    {
...
bool RedisManager::hash_dels(const char *hashkey, const char *subkeys)
{
    ...
    redisReply *replay = (redisReply *)redisCommand(pRedisCtx, cmd.c_str());
    mtx.unlock();
    bool success = (replay && (replay->integer >= 0));
    freeReplyObject(replay);
    return success;
}
...
int RedisManager::hash_increase(const char *hashkey, const char *subkey, int n)
{
    ...
    mtx.unlock();
    int value = (replay && (replay->type == REDIS_REPLY_INTEGER)) ? replay->integer : 0;
    freeReplyObject(replay);
    return value;
}
...
string RedisManager::hash_get(const char *hashkey, const char *subkey)
{
    ...
    mtx.unlock();
    string res = (replay && (replay->type == REDIS_REPLY_STRING)) ? string(replay->str, replay->len) : string();
    freeReplyObject(replay);
    return res;
}
...
size_t RedisManager::hash_len(const char *hashkey, size_t len1)
{
    ...
    mtx.unlock();
    size_t len = (replay && replay->type == REDIS_REPLY_INTEGER) ? replay->integer : 0;
